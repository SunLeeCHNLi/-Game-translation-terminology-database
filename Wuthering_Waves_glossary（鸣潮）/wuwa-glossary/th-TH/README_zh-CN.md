# 鸣潮（Wuthering Waves）术语库 — 泰文（`th-TH`）

[← 返回游戏总说明](../../README.md) ｜ [← wuwa-glossary 子库说明](../README.md)

本目录是**以 `th-TH`（泰文）为目标语言**的鸣潮（Wuthering Waves）术语库。共 **123,230** 条词条、**644,759** 行对照（本目录 23 个 CSV 的数据行数合计），目录体积约 **91.4 MiB**。所有 CSV 的 `tgt_lng` 列固定为 `th-TH`；`source` 列收录其余 9 种语言（`zh-CN`（简体中文）、`zh-TW`（繁體中文）、`en-US`（English）、`ja-JP`（日本語）、`ko-KR`（한국어）、`fr-FR`（Français）、`de-DE`（Deutsch）、`es-ES`（Español）、`pt-BR`（Português））的写法，因此同一条游戏文本可以从任意一种语言匹配到本语言的译名。
...
| 分类 | 主题 | 条数 | 对照行 |
| --- | --- | --- | --- |
| `characters.csv` | 角色名称 | 1,230 | 5,216 |
| `weapons.csv` | 武器名称 | 820 | 3,000 |
| `echoes.csv` | 声骸 | 1,000 | 6,125 |
| `skills.csv` | 技能 | 5,344 | 30,650 |
| `resonant-chains.csv` | 共鸣链 | 784 | 6,116 |
| `quests.csv` | 任务 | 2,807 | 14,558 |
| `dungeons.csv` | 关卡与挑战 | 1,910 | 10,460 |
| `regions.csv` | 地区与地图 | 2,229 | 16,019 |
| `factions.csv` | 阵营与势力 | 8 | 44 |
| `items.csv` | 道具与材料 | 8,384 | 54,950 |
| `monsters.csv` | 怪物与生物 | 685 | 4,543 |
| `npcs.csv` | NPC 与说话人 | 14,172 | 63,906 |
| `achievements.csv` | 成就 | 2,563 | 22,170 |
| `activities.csv` | 活动与玩法 | 9,531 | 63,508 |
| `buffs.csv` | 增益与效果 | 270 | 2,029 |
| `voice-lines.csv` | 角色语音 | 7,374 | 32,117 |
| `archives.csv` | 档案与读物 | 839 | 6,571 |
| `terms.csv` | 术语与百科 | 1,672 | 11,950 |
| `system.csv` | 系统文本 | 9,756 | 65,406 |
| `ui.csv` | UI 文本 | 13,866 | 78,162 |
| `tutorials.csv` | 教程与引导 | 6,253 | 32,056 |
| `story.csv` | 剧情文本 | 29,841 | 101,876 |
| `other.csv` | 其他 | 1,892 | 13,327 |
| **合计** | **23 个类目** | **123,230** | **644,759** |
