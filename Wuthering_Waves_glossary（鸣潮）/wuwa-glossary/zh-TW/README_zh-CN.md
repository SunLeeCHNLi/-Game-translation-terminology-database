# 鸣潮（Wuthering Waves）术语库 — 繁体中文（`zh-TW`）

[← 返回游戏总说明](../../README.md) ｜ [← wuwa-glossary 子库说明](../README.md)

本目录是**以 `zh-TW`（繁体中文）为目标语言**的鸣潮（Wuthering Waves）术语库。共 **123,230** 条词条、**645,529** 行对照（本目录 23 个 CSV 的数据行数合计），目录体积约 **55.7 MiB**。所有 CSV 的 `tgt_lng` 列固定为 `zh-TW`；`source` 列收录其余 9 种语言（`zh-CN`（简体中文）、`en-US`（English）、`ja-JP`（日本語）、`ko-KR`（한국어）、`fr-FR`（Français）、`de-DE`（Deutsch）、`es-ES`（Español）、`pt-BR`（Português）、`th-TH`（ภาษาไทย））的写法，因此同一条游戏文本可以从任意一种语言匹配到本语言的译名。
