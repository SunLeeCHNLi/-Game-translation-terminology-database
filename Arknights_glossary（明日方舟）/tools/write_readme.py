...
HERE = os.path.dirname(os.path.abspath(__file__))         # <game>/tools
REPO = os.path.dirname(HERE)                              # <game>
OUTDIR = os.path.join(REPO, "glossary")                   # <game>/glossary
SUMMARY = os.path.join(HERE, "_summary.json")             # <game>/tools/_summary.json
README = os.path.join(OUTDIR, "README.md")

S = json.load(io.open(SUMMARY, encoding="utf-8"))
cats = S["categories"]
...
w("")
w("```")
w("Arknights_glossary（明日方舟）/")
w("├── glossary/                 # 术语库数据产品")
w("│   ├── README.md             # 本说明（由 tools/write_readme.py 生成）")
w("│   ├── zh-CN/                # 目标语言 = 简体中文")
w("│   │   ├── 01_干员名称.csv")
w("│   │   ├── 02_干员异格.csv")
w("│   │   ├── ...")
...
w("│   └── ko-KR/   (同结构)")
w("├── tools/                    # 生成脚本与元数据")
w("│   ├── build_glossary.py     # 读取 _data/gamedata，输出 glossary/")
w("│   ├── write_readme.py       # 依据 _summary.json 生成 glossary/README.md")
w("│   └── _summary.json         # 各分类词条数与覆盖率统计")
w("└── README.md                 # 游戏总说明（中文）")
w("```")
w("")
w("## 文件格式")
w("")
...
w("")
w("（「词条数」= 同一分类下参与对齐的**唯一 ID 数**；「五语齐全」= 5 个区服都取到文本的 ID 数；")
w("「各语言分布」表内为 **CSV 数据行数**——同一词条会因多语言 source 产生多行，且 source 与 target")
w("写法完全相同的行不会写入，因此各语言行数略有差异。）")
w("")
w("## 数据来源与版本")
w("")
...
w("")
w("```")
w("python tools/build_glossary.py     # 读取 <游戏根>/_data/gamedata，输出 <游戏根>/glossary/")
w("python tools/write_readme.py       # 依据 tools/_summary.json 重新生成本文件")
w("```")
w("")
w("> `_data/gamedata` 是外部数据目录（各服官方客户端解包表格），**未随仓库分发**，")
w("> 需先按「数据来源与版本」一节自行准备后才能复现。")
w("")
