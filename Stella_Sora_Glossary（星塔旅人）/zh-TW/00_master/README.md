# 《星塔旅人》Stella Sora 术语库 — 繁體中文（`zh-TW`）

> 本文件位于 `zh-TW/00_master/`，描述的是**上级语言目录 `zh-TW/`**（以繁體中文为目标语言的整套术语库）。
> 语言级说明见 `../README.md`（繁體中文）与 `../README_zh-CN.md`（简体中文），游戏级总说明见 `../../README.md`。

本语言目录是**以 `zh-TW` 为目标语言**的术语库：每一条都是「其他语言词条 → 繁體中文」的对照，
共 **12292** 条词条、**46052** 行对照（`tgt_lng` 列固定为 `zh-TW`）。
