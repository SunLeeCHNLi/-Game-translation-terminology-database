# 《星塔旅人》Stella Sora 术语库 — 한국어（`ko-KR`）

> 本文件位于 `ko-KR/00_master/`，描述的是**上级语言目录 `ko-KR/`**（以한국어为目标语言的整套术语库）。
> 语言级说明见 `../README.md`（한국어）与 `../README_zh-CN.md`（简体中文），游戏级总说明见 `../../README.md`。

本语言目录是**以 `ko-KR` 为目标语言**的术语库：每一条都是「其他语言词条 → 한국어」的对照，
