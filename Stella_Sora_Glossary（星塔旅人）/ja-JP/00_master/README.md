# 《星塔旅人》Stella Sora 术语库 — 日本語（`ja-JP`）

> 本文件位于 `ja-JP/00_master/`，描述的是**上级语言目录 `ja-JP/`**（以日本語为目标语言的整套术语库）。
> 语言级说明见 `../README.md`（日本語）与 `../README_zh-CN.md`（简体中文），游戏级总说明见 `../../README.md`。

本语言目录是**以 `ja-JP` 为目标语言**的术语库：每一条都是「其他语言词条 → 日本語」的对照，
共 **12289** 条词条、**46426** 行对照（`tgt_lng` 列固定为 `ja-JP`）。
