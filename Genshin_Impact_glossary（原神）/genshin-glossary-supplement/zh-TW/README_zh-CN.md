# 原神术语库（补充词库） — 繁体中文（`zh-TW`）

[← 返回游戏总说明](../../README.md) · [繁體中文](README.md)

本目录是**以 `zh-TW`（繁体中文）为目标语言**的原神术语库**补充词库**：全局 `genshin-glossary/` 主词库按 27 个类目统计共 **8,186** 条词条，本补充库收录主词库未包含的词条，本语言共 **12,221** 行对照（另有别名 `_variants.csv` **332** 行）。`tgt_lng` 列固定为 `zh-TW`；每行的 `source` 是同一词条在**其余 3 种语言（`zh-CN`、`en-US`、`ja-JP`）中的某一种**的写法，`target` 是繁体中文译名。

## 与主词库的关系

- 本库只包含**主词库中没有的** `source/target/tgt_lng` 组合，因此可与 `genshin-glossary/zh-TW/` 直接**叠加使用**，不会产生重复条目。
...
| 分类 | 文件 | 主题（上游来源） | 行数 |
| --- | --- | --- | ---: |
| characters（角色） | `characters.csv` | characters-*（蒙德/璃月/稻妻/须弥/枫丹/纳塔/挪德卡莱/至冬/坎瑞亚/愚人众等） | 3,796 |
...
| 文件 | 说明 | 行数 |
| --- | --- | ---: |
| `_variants.csv` | 别名/俗称/常见误写，作为额外 `source` 补充 | 332 |

### 额外类目（`extra/`）

| 分类 | 文件 | 说明 | 行数 |
| --- | --- | --- | ---: |
| quests（任务） | `extra/quests.csv` | 任务名称（魔神/世界/传说/每日/部族等） | 1,652 |
| events（活动） | `extra/events.csv` | 活动名称 | 1,705 |
| objects（物件） | `extra/objects.csv` | 场景物件 | 447 |
| system（系统） | `extra/system.csv` | 系统与玩法术语 | 343 |
| archives（档案） | `extra/archives.csv` | 档案资料 | 347 |
| story（剧情） | `extra/story.csv` | 剧情与章节 | 282 |
| facilities（设施） | `extra/facilities.csv` | 设施与建筑 | 226 |
| organizations（组织） | `extra/organizations.csv` | 组织与势力 | 205 |
| dialogue（对白） | `extra/dialogue.csv` | 对白用语 | 115 |
| sereniteapot（尘歌壶） | `extra/sereniteapot.csv` | 尘歌壶 | 32 |
| **小计** | 10 个文件 | — | **5,354** |

**本目录合计：20 个文件、12,221 行对照（含别名 332 行）。**
...
- **语言标签**：本目录 `tgt_lng` 固定为 `zh-TW`，表示目标语言；`source` 可能是 `zh-CN`、`en-US`、`ja-JP` 中的任意一种。
