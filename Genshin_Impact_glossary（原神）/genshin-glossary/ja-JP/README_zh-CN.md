...
本目录是原神（Genshin Impact）术语库中**以 `ja-JP` 为目标语言**的主词库：共 **8,186** 条去重词条、**89,490** 行对照（其中 17 个主类目 **63,173** 行，TCG 10 个子类目 **26,317** 行）。27 个 CSV 的 `tgt_lng` 列固定为 `ja-JP`，`source` 列收录其余 13 种语言中某一种的写法，`target` 列为该语言的游戏内译名。
...
`source` = 同一游戏对象在其余 13 种语言中某一种的写法，`target` = 本目录目标语言的名称，`tgt_lng` = 目标语言标签（本目录恒为 `ja-JP`）。这些文件可直接作为术语库导入 CAT 工具（Trados、memoQ、Phrase 等）或沉浸式翻译等术语匹配插件。
...
| `TCG/action-cards.csv` | 行动牌 | 927 | 9,534 |
| `TCG/character-cards.csv` | 角色牌 | 149 | 929 |
| `TCG/enemy-cards.csv` | 敌人牌 | 134 | 1,125 |
| `TCG/summons.csv` | 召唤物 | 152 | 1,197 |
| `TCG/status-effects.csv` | 状态效果 | 1,159 | 11,271 |
...
| **TCG（10 个文件）** | — | **2,743** | **26,317** |
| **合计（27 个文件）** | — | **8,186** | **89,490** |
...
- **语言标签**：本目录所有文件的 `tgt_lng` 列固定为 `ja-JP`（genshin-db 内部名 `Japanese`）。
