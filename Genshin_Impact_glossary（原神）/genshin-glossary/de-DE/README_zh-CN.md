...
本目录是原神（Genshin Impact）术语库中**以 `de-DE` 为目标语言**的主词库：共 **8,186** 条去重词条、**89,362** 行对照（其中 17 个主类目 **63,172** 行，TCG 10 个子类目 **26,190** 行）。27 个 CSV 的 `tgt_lng` 列固定为 `de-DE`，`source` 列收录其余 13 种语言中某一种的写法，`target` 列为该语言的游戏内译名。
...
`source` = 同一游戏对象在其余 13 种语言中某一种的写法，`target` = 本目录目标语言的名称，`tgt_lng` = 目标语言标签（本目录恒为 `de-DE`）。这些文件可直接作为术语库导入 CAT 工具（Trados、memoQ、Phrase 等）或沉浸式翻译等术语匹配插件。
...
- **语言标签**：本目录所有文件的 `tgt_lng` 列固定为 `de-DE`（genshin-db 内部名 `German`）。
