...
[← 返回游戏总说明](../../README.md) · [Русский](README.md)

本目录是**以 `ru-RU`（俄文）为目标语言**的原神术语库：按 27 个类目统计，全局共 **8,186** 条词条，本语言共 **89,418** 行对照。`tgt_lng` 列固定为 `ru-RU`；每行的 `source` 是同一词条在**其余 13 种语言中的某一种**的写法，`target` 是俄文译文。文件按**类目**分列，每个类目一个 CSV。
...
- **语言标签**：本目录 `tgt_lng` 固定为 `ru-RU`，表示目标语言；`source` 可能是其余 13 种语言（`zh-CN`、`zh-TW`、`en-US`、`ja-JP`、`ko-KR`、`fr-FR`、`de-DE`、`es-ES`、`pt-BR`、`it-IT`、`tr-TR`、`th-TH`、`vi-VN`）中的任意一种。
