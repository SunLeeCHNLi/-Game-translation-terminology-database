...
[← 返回游戏总说明](../../README.md) · [Türkçe](README.md)

本目录是**以 `tr-TR`（土耳其文）为目标语言**的原神术语库：按 27 个类目统计，全局共 **8,186** 条词条，本语言共 **89,423** 行对照。`tgt_lng` 列固定为 `tr-TR`；每行的 `source` 是同一词条在**其余 13 种语言中的某一种**的写法，`target` 是土耳其文译文。文件按**类目**分列，每个类目一个 CSV。
...
| achievements（成就） | `achievements.csv` | 1,548 | 19,463 |
...
| 子类目 | 文件 | 词条数 | 行数 |
| --- | --- | ---: | ---: |
| action-cards（行动牌） | `TCG/action-cards.csv` | 927 | 9,592 |
...
| status-effects（状态效果） | `TCG/status-effects.csv` | 1,159 | 11,204 |
...
- **语言标签**：本目录 `tgt_lng` 固定为 `tr-TR`，表示目标语言；`source` 可能是其余 13 种语言（`zh-CN`、`zh-TW`、`en-US`、`ja-JP`、`ko-KR`、`fr-FR`、`de-DE`、`es-ES`、`ru-RU`、`pt-BR`、`it-IT`、`th-TH`、`vi-VN`）中的任意一种。
